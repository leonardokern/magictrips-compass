-- =============================================================================
-- 045 — Auto-aprovação por Admin/Gerente + exclusão de vendas
-- =============================================================================
-- Mudanças no fluxo de venda:
--   1. Quando o criador tem permissão de aprovar (Admin/Gerente), a venda já
--      é criada com status 'aprovado' (sem passar por pendente_validacao).
--   2. Quando o usuario_id da venda for diferente do criador, o responsável
--      recebe lembrete avisando que uma venda foi cadastrada em seu nome.
--   3. Admin/Gerente pode excluir vendas (com auditoria) via excluir_venda.
-- =============================================================================

-- ── 1. Política de DELETE em vendas (Admin/Gerente da mesma empresa) ─────────
DROP POLICY IF EXISTS vendas_delete ON public.vendas;
CREATE POLICY vendas_delete ON public.vendas FOR DELETE USING (
  mesma_empresa(empresa_id) AND (is_administrador() OR is_gerente())
);

-- ── 2. criar_venda_completa: auto-aprovação + notificação ao responsável ────
...
BEGIN
  ...
  -- Upsert de cliente novo (PF ou PJ)
  ...
  -- Identificador sequencial atômico
  UPDATE empresas
  ...
  -- Insere venda (com status final calculado)
  INSERT INTO vendas (
    ...
    v_indicacao, v_comissao_perc, v_pax, v_status_inicial,
    v_observacoes, v_identificador,
    CASE WHEN v_auto_aprovado THEN v_uid ELSE NULL END,
    CASE WHEN v_auto_aprovado THEN NOW() ELSE NULL END
  )
  ...
  VALUES (v_uid, v_empresa_id, 'criar', 'venda', v_venda_id, p_payload);

  IF v_auto_aprovado THEN
    -- Audit log de aprovação (mesmo registro de criação + aprovação)
    INSERT INTO audit_logs (usuario_id, empresa_id, acao, entidade, entidade_id, dados_depois)
    VALUES (
      v_uid, v_empresa_id, 'aprovado', 'vendas', v_venda_id,
      jsonb_build_object(
        'origem',         'criacao_direta',
        'aprovado_por',   v_uid,
        'data_aprovacao', NOW()
      )
    );

    -- Notifica o responsável apenas se for outro usuário
    IF v_usuario_id <> v_uid THEN
      INSERT INTO lembretes (
        ...
        'venda_aprovada', 'venda', v_venda_id,
        v_usuario_id, v_empresa_id,
        CURRENT_DATE,
        'Uma venda (' || v_identificador || ') foi cadastrada em seu nome.',
        'pendente'
      );
    END IF;
  ELSE
    -- Fluxo legado: agente cria pendente → notifica aprovadores
...
-- ── 3. excluir_venda: hard delete com auditoria (Admin/Gerente) ──────────────
